...
(function (window) {
    ...
    const SHEET_HEADERS = ['التاريخ', 'رقم الفاتورة', 'الاسم', 'الهاتف', 'الخدمات', 'الاجمالي', 'الحاله', 'كاش', 'بنفت', 'تاريخ الدفع', 'الدفع الشهري'];
    ...
    function arrToInv(line) {
        ...
        return {
            ...
            paidAt: parseDateFlex(get(9)), createdAt: new Date().toISOString(), readyAt: '', cancelledAt: ''
        };
    }
    ...
    async function importInvoicesCSV(ev) {
        ...
        try {
            ...
            for (let r = 1; r < rows.length; r++) {
                ...
                const found = existing.find(x => (x.invNo || 0) === inv.invNo);
                if (found) {
                    Object.assign(found, inv);
                    updated++;
                } else {
                    inv.id = Date.now() + Math.floor(Math.random() * 1000);
                    existing.push(inv);
                    added++;
                }
            }
            ...
            await loadCaches();
            renderVert();
            toast(t('csv_import_ok') + ' (+' + added + ' / ~' + updated + ')', 'success');
        } catch (e) {
            console.error(e);
    ...
    async function sheetSync(push) {
        ...
        try {
            ...
            if (!push && Array.isArray(data.invoices)) {
                const existing = App.state.invoices || [];
                for (const line of data.invoices) {
                    if (!Array.isArray(line) || line.length < 11 || !String(line[1]).trim()) continue;
                    const inv = arrToInv(line);
                    const found = existing.find(x => (x.invNo || 0) === inv.invNo);
                    if (found) Object.assign(found, inv);
                    else { inv.id = Date.now() + Math.floor(Math.random() * 1000); existing.push(inv); }
                }
                for (const x of existing) await DB.put('invoices', x);
                App.state.invoices = await DB.getAll('invoices');
                await loadCaches();
