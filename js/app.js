...
(function (window) {
    ...
    async function init() {
        try {
            if (window.SUPA && window.SUPA.enabled) await window.SUPA.bootstrap();
            await loadCaches();
        } catch (e) {
                ...
                <div class="panel">
                    ...
                    <div class="field">
                        <input id="posPhone" value="${esc(S.customer ? S.customer.phone : '')}" placeholder="${esc(t('search_customer'))}"
                               onkeyup="App.posPhoneKey(event)" onchange="App.posPhoneDone()">
                    </div>
                    <input id="posName" value="${esc(S.customer ? S.customer.name : '')}" placeholder="${esc(t('customer_name'))}">
                    <div class="field">
    ...
    async function posSave(printIt) {
        ...
        let cust = null;
        if (phone) {
            cust = App.state.customers.find(c => c.phone === phone);
            if (cust) {
                cust.name = name;
        ...
        const inv = {
            ...
            phone: phone || '',
            items: S.items.map(it => ({ name: it.name, qty: it.qty, mode: it.mode, price: it.price })),
    ...
    async function posSaveAndSend() {
        ...
        if (!cfg.evoServerUrl || !cfg.evoInstance) {
            toast(t('toast_wa_missing_cfg'), 'warn');
            return;
        }
        const itemsTxt = WA.itemsText(inv, App.state.lang);
        try {
            await WA.sendInvoiceMessage(cfg, inv, cfg.shopName || t('appName'), itemsTxt);
            toast(t('toast_wa_sent'), 'success');
        } catch (e) {
            toast(t('wa_failed'), 'error');
        }
    }
    ...
    function viewInvoices(el) {
        ...
        el.innerHTML = `
            ...
            <div class="table-wrap panel" style="margin-top:14px">
                <table class="table">
                    <thead><tr>
                        <th>${esc(t('invoice_no'))}</th><th>${esc(t('name'))}</th>
    ...
    function invoiceRow(i) {
        ...
        return `<tr>
            <td class="strong">#${esc(String(i.invNo))}</td>
            <td>${esc(i.custName || '')}<br><span class="dim">${esc(i.phone || '')}</span></td>
            <td class="strong">${cur(i.netTotal)}</td>
            <td class="dim">${esc(DB.fmtDate(i.createdAt))}<br>${statusBadge(i.status)}</td>
            <td class="dim">${payTxt}</td>
    ...
    async function markReady(id) {
        ...
        const cfg = App.state.settings;
        const auto = cfg.evoAutoSend === '1' || cfg.evoAutoSend === true;
        if (auto && cfg.evoServerUrl && cfg.evoInstance && inv.phone) {
            try {
                await WA.sendReadyMessage(cfg, inv, cfg.shopName || t('appName'), WA.itemsText(inv, App.state.lang));
    ...
    function sendInvoiceWA(id) {
        ...
        if (!cfg.evoServerUrl || !cfg.evoInstance) {
            // fallback to wa.me
            const cc = WA.cleanNumber(inv.phone, cfg.countryCode);
            const msg = encodeURIComponent(`${t('wa_greeting')} ${inv.custName}, ${t('invoice')} #${inv.invNo} = ${fmt(inv.netTotal)} ${cfg.currency || 'BHD'}`);
            window.open(`https://wa.me/${cc}?text=${msg}`, '_blank');
            return;
        }
        WA.sendInvoiceMessage(cfg, inv, cfg.shopName || t('appName'), WA.itemsText(inv, App.state.lang))
            .then(() => toast(t('toast_wa_sent'), 'success'))
            .catch(() => toast(t('wa_failed'), 'error'));
    }
        ...
        modalOpen(`
            ...
            <div class="modal-field">
                ...
                <div class="detail-items">
                    ${inv.items.map(x => `<div class="bill-row"><span>${esc(x.name)} ×${x.qty} <i class="dim">(${esc(x.mode)})</i></span><span>${cur(x.price)}</span></div>`).join('') || '<div class="empty">—</div>'}
                </div>
            </div>
            <div class="detail-grid">
    ...
    function currentEvoCfg() {
        return {
            serverUrl: document.getElementById('sEvoServer').value.trim(),
    ...
    async function saveSettings() {
        const pairs = [
            ...
            { key: 'evoServerUrl', value: document.getElementById('sEvoServer').value.trim() },
    ...
    function printInvoice(inv) {
        ...
        const html = `
        <html>
        <head><meta charset="utf-8"><title>Invoice #${esc(String(inv.invNo))}</title><style>${css}</style></head>
        <body>
            <div class="inv">
                <div class="header">
    ...
    Object.assign(App, {
        ...
        printInvoiceById,
        posPhoneKey, posPhoneDone, posAdd, posRemove, posClear, posRecalc, posSave, posSaveAndSend
    });
