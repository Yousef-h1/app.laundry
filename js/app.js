...
(function (window) {
    ...
    function viewDashboard(el) {
        ...
        const activeSubIds = new Set(subs.filter(s => s.status === 'active').map(s => s.id));
        subPays.forEach(p => { if (activeSubIds.has(p.subId) && inMonth(p.createdAt, now)) monthSubRev += Number(p.amount || 0); });
        ...
        subs.forEach(s => { if (s.status === 'active') activeCustIds.add(s.custId !== undefined && s.custId !== null ? s.custId : s.id); });
        const activeSubs = activeCustIds.size;
        const recent = [...inv].sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, 8);

        const stats = [
            [t('stat_total_customers'), App.state.customers.length, 'blue'],
        ...
        body.innerHTML = S.items.length ? S.items.map((it, idx) => `
            <div class="bill-row">
                ...
                <div class="bill-line2">
                    <span>${esc(it.mode)} • ${esc(itemUrg(it))}</span>
    ...
    function posPhoneDone() {
        ...
        const pad = phone.replace(/\D/g, '');
        const cust = App.state.customers.find(c =>
            String(c.phone || '').replace(/\D/g, '') === pad && (!name || String(c.name || '').trim() === name))
            || (name ? App.state.customers.find(c => String(c.phone || '').replace(/\D/g, '') === pad) : null);
        if (cust) {
    ...
    function posSug() {
        ...
        const exact = App.state.customers.find(c =>
            String(c.phone || '').replace(/\D/g, '') === phoneDigits && String(c.name || '').trim() === name);
        if (exact) { wrap.style.display = 'none'; wrap.innerHTML = ''; return; }
        const byName = !!name;
        const opts = (App.state.customers || []).filter(c => {
            const n = String(c.name || '').toLowerCase();
            const p = String(c.phone || '').replace(/\D/g, '');
            return byName ? n.indexOf(name.toLowerCase()) > -1
                          : p.indexOf(phoneDigits) > -1;
        }).slice(0, 8);
        ...
        if (!App._posSugBound) {
            App._posSugBound = true;
            App._posClickFn = (ev) => posSugOutside(ev);
            App._posEscFn = (ev) => { if (ev.key === 'Escape') { App.posHideSug(); App._posSugBound = false; document.removeEventListener('click', App._posClickFn); document.removeEventListener('touchstart', App._posClickFn); document.removeEventListener('keydown', App._posEscFn); } };
            document.addEventListener('click', App._posClickFn);
            document.addEventListener('touchstart', App._posClickFn);
            document.addEventListener('keydown', App._posEscFn);
        }
    }
    ...
    function posSugOutside(ev) {
        const box = document.getElementById('posSug');
        if (!box || box.style.display === 'none') { App._posSugBound = false; document.removeEventListener('click', App._posClickFn); document.removeEventListener('touchstart', App._posClickFn); document.removeEventListener('keydown', App._posEscFn); return; }
        const t = ev.target;
        if (t && (t.id === 'posPhone' || t.id === 'posName')) return;
        if (box && box.contains && box.contains(t)) return;
        App.posHideSug();
        App._posSugBound = false;
        document.removeEventListener('click', App._posClickFn);
        document.removeEventListener('touchstart', App._posClickFn);
        document.removeEventListener('keydown', App._posEscFn);
    }
    ...
    async function posSave(printIt) {
        ...
        let invNo = null;
        if (editId) {
            const ex = App.state.invoices.find(x => x.id === editId);
            invNo = (ex && Number(ex.invNo)) || null;
        }
        if (!invNo) {
            try { invNo = await WA.nextInvoiceNo(App.state.settings || {}); } catch (e) { invNo = null; }
        }
        if (!invNo) invNo = await DB.nextInvoiceNumber();
        const inv = {
            invNo,
    ...
    function viewInvoices(el) {
        ...
        else if (filter === 'all') list = list.filter(i =>
            isSameDay(i.createdAt, now) || i.status === 'pending' || i.status === 'ready' ||
            (i.status === 'paid' && isSameDay(i.paidAt, now)));
        else if (filter === 'pending') list = list.filter(i => i.status === 'pending' || i.status === 'ready');
        else if (filter === 'ready') list = list.filter(i => i.status === 'ready');
        else if (filter === 'paid') list = list.filter(i => i.status === 'paid' && isSameDay(i.paidAt, now));
        ...
        el.innerHTML = `
            <div class="panel">
                <div class="row2" style="margin-bottom:10px">
                    <button class="btn btn-primary" id="fullSyncBtn" onclick="App.fullSync()">${esc(t('sync_btn'))}</button>
                    <span class="hint" style="line-height:32px">${esc(t('sync_btn_hint'))}</span>
                </div>
                <div class="filters">
                    ${filters.map(f => `
    ...
    function invoiceRow(i) {
        ...
        actions.push(`<button class="btn btn-green btn-sm" onclick="App.sendInvoiceWA(${i.id})">${esc(t('send_wa_invoice'))}</button>`);
    ...
    function mergeItemPrices(localItems, sheetItems) {
        if (!Array.isArray(sheetItems) || !sheetItems.length) return sheetItems || [];
        if (sheetItems.some(it => Number(it.price) > 0)) return sheetItems;
        const fixed = (it) => String(it.name || '')
            .replace(/\s*\(x\d+(?:\.\d+)?\)\s*$/i, '').trim();
        const src = Array.isArray(localItems) ? localItems : [];
        return sheetItems.map(it => {
            let m = src.find(s => String(s.name || '') === String(it.name || '')
                && String(s.mode || '') === String(it.mode || '')
                && (String(s.qty) === String(it.qty) || (!s.qty && !it.qty)));
            if (!m) {
                const fn = fixed(it);
                m = src.find(s => String(s.name || '') === fn);
            }
            return m && Number(m.price) > 0 ? Object.assign({}, it, { price: m.price }) : it;
        });
    }
    ...
    async function mergeSheetInvoices(lines) {
        ...
        for (const line of lines) {
            ...
            const n = Number(inv.invNo || 0);
            if (n && !byNo.has(n)) byNo.set(n, inv);
        }
        let added = 0, updated = 0, removed = 0;
        const kept = [];
        for (const x of existing) {
            const n = Number(x.invNo || 0);
            const sh = byNo.get(n);
            const shId = sh && sh.id !== undefined && sh.id !== null && sh.id !== '';
            const sameId = shId && x.id !== undefined && x.id !== null && x.id !== ''
                && String(x.id) === String(sh.id);
            if (sh && (sameId || !shId)) {
                const hadId = x.id;
                const hadItems = x.items;
                const hadNet = Number(x.netTotal || 0);
                const hadTotal = Number(x.total || 0);
                Object.assign(x, sh);
                x.items = mergeItemPrices(hadItems, sh.items);
                x.createdAt = sh.createdAt;
                if (!x.id || (x.id !== undefined && x.id !== null && String(x.id) === '')) x.id = hadId;
                if ((!Number(x.netTotal) || !Number(x.total)) && (hadNet || hadTotal)) {
                    x.netTotal = hadNet || hadTotal;
                    x.total = hadTotal || hadNet;
                }
                updated++;
            } else if (sh) {
                removed++; /* stale local duplicate whose number/id disagree with the sheet */
            } else {
                kept.push(x); /* genuinely-new local invoice, pushed later */
            }
        }
        for (const [n, sh] of byNo) {
            const has = kept.some(x => Number(x.invNo || 0) === n);
            if (has) continue;
            const inv = Object.assign({}, sh);
            if (!inv.id) inv.id = Date.now() + Math.floor(Math.random() * 100000);
            if (!inv.createdAt) inv.createdAt = DB.isoNow();
            kept.push(inv);
            added++;
        }
        if (added || updated || removed) {
            for (const x of kept) await DB.put('invoices', x);
            App.state.invoices = await DB.getAll('invoices');
            await upsertCustomersFromInvoices(kept);
            await loadCaches();
            renderVert();
        }
        await alignLocalCounter(App.state.invoices);
        return { added, updated, removed };
    }

    async function alignLocalCounter(invList) {
        try {
            if (!invList || !invList.length) return;
            let mx = 0;
            invList.forEach(i => { const n = parseInt(i.invNo, 10); if (!isNaN(n) && n > mx) mx = n; });
            const cur = parseInt(App.state.settings.nextInvNo, 10);
            if (mx + 1 > (isNaN(cur) ? 1001 : cur)) {
                await DB.setSetting('nextInvNo', (mx + 1).toString());
                App.state.settings.nextInvNo = String(mx + 1);
            }
        } catch (e) { /* ignore */ }
    }
    ...
    function svcText(inv) {
        ...
        return inv.items.map(it => {
            ...
            const price = Number(it.price) > 0 ? ' @' + Number(it.price) : '';
            return n + mode + ' (x' + (it.qty || 1) + ')' + price;
        }).join(', ');
    }
    function money3(n) {
    ...
    function svcToItems(s) {
        ...
        return String(s).split(',').map(tok => {
            tok = tok.trim();
            if (!tok) return null;
            let m = tok.match(/^\s*(.*?)\s*\(([^)]*)\)\s*\(x([\d.]+)\)(?:\s*@([\d.]+))?\s*$/);
            let name, mode, qty, price;
            if (m) { name = m[1]; mode = m[2]; qty = parseFloat(m[3]) || 1; price = parseFloat(m[4]) || 0; }
            else if ((m = tok.match(/^\s*(.*?)\s*\(([^)]*)\)\s*x([\d.]+)(?:\s*@([\d.]+))?\s*$/))) { name = m[1]; mode = m[2]; qty = parseFloat(m[3]) || 1; price = parseFloat(m[4]) || 0; }
            else if ((m = tok.match(/^\s*(.*?)\s*\(x([\d.]+)\)(?:\s*@([\d.]+))?\s*$/))) { name = m[1]; mode = ''; qty = parseFloat(m[2]) || 1; price = parseFloat(m[3]) || 0; }
            else if ((m = tok.match(/^\s*(.*?)\s*\(([^)]*)\)\s*@([\d.]+)\s*$/))) { name = m[1]; mode = m[2]; qty = 1; price = parseFloat(m[3]) || 0; }
            else { name = tok; mode = ''; qty = 1; price = 0; }
            return { name, mode, qty, price, urgent: 0 };
        }).filter(Boolean);
    }
    ...
    function arrToInv(line) {
        ...
        const items = svcToItems(get(4));
        const totQty = items.reduce((a, x) => a + (parseFloat(x.qty) || 1), 0);
        if (items.length && totQty > 0 && items.every(x => !Number(x.price))) {
            const unit = tot / totQty;
            items.forEach(x => { x.price = Math.round(unit * (parseFloat(x.qty) || 1) * 1000) / 1000; });
        }
        const out = {
        ...
        if (dm) dCreated = new Date(Number(dm[1]), Number(dm[2]) - 1, Number(dm[3])).toISOString();
        if (!dCreated) {
            const fx = parseDateFlex(d0);
            if (fx) dCreated = fx;
        }
        if (!dCreated) {
            const iso = new Date(d0);
            dCreated = isNaN(iso) ? '' : iso.toISOString();
        }
        out.createdAt = dCreated || new Date().toISOString();
    ...
    async function fullSync() {
        if (_syncBusy.on) { toast(t('sync_running'), 'warn'); return; }
        _syncBusy.on = true;
        const btn = document.getElementById('fullSyncBtn');
        if (btn) { btn.disabled = true; btn.textContent = t('sync_running'); }
        try {
            const s = App.state.settings || {};
            const url = String(s.gsheetUrl || s.evoPushUrl || '').trim();
            ...
            let pushOk = true;
            try {
                const pr = await fetchWithTimeout(u, {
                    method: 'POST',
                    headers: { 'Content-Type': 'text/plain;charset=utf-8' },
                    body: JSON.stringify({ action: 'push', invoices: (App.state.invoices || []).map(invToRow) })
                }, 30000);
                const pj = await pr.json();
                pushOk = pr.ok && pj && pj.status === 'success';
            } catch (e) { pushOk = false; }
            let m = null;
            try {
                const rr = await fetchWithTimeout(u + (u.indexOf('?') > -1 ? '&' : '?') + 'action=export', { method: 'GET' }, 30000);
                const data = await rr.json();
                if (data && data.status === 'success' && Array.isArray(data.invoices)) {
                    m = await mergeSheetInvoices(data.invoices);
                }
            } catch (e) { m = null; }
            if (!pushOk && !m) throw new Error('sync failed');
            const added = m ? m.added : 0, updated = m ? m.updated : 0;
            const msg = t('sync_done').replace('{a}', added).replace('{u}', updated);
            toast(msg + (pushOk ? '' : ' — ' + t('sync_push_fail')), pushOk ? 'success' : 'warn');
            if ('serviceWorker' in navigator) {
                try {
                    ...
                    if (reg) {
                        ...
                        if (reg.active && prev && reg.active !== prev) {
                            setTimeout(() => { location.reload(); }, 500);
                        }
                    }
                } catch (e) { /* self-update is optional */ }
            }
        } catch (e) {
            toast(t('sync_fail'), 'error');
        } finally {
            _syncBusy.on = false;
            if (btn) { btn.disabled = false; btn.textContent = t('sync_btn'); }
        }
    }
    ...
    async function sheetSync(push) {
        ...
        let url = (inp ? inp.value : App.state.settings.gsheetUrl || '').trim()
            || String(App.state.settings.evoPushUrl || '').trim();
    ...
    function printInvoice(inv) {
        ...
        if (isIOS && window.WA && WA.drawInvoicePng) {
            try {
                const dataUrl = WA.drawInvoicePng(inv, App.state.settings);
                if (dataUrl) { printInvoiceImage(dataUrl); return; }
            } catch (e) { /* fall back to html print */ }
        }
    ...
    function printInvoiceImage(dataUrl) {
        const full = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Receipt</title>
            <style>
                @page{size:58mm auto;margin:0}
                *{-webkit-print-color-adjust:exact;print-color-adjust:exact}
                html,body{margin:0;padding:0;background:#fff}
                .rc{width:48mm;margin:0 auto;padding:0}
                img{width:48mm;height:auto;display:block;margin:0}
            </style></head><body><div class="rc"><img src="${dataUrl}"></div></body></html>`;
    ...
    function printCss() {
        return `
            @page{size:58mm auto;margin:0}
            *{-webkit-print-color-adjust:exact;print-color-adjust:exact}
            body{font-family:'Segoe UI',Tahoma,Arial,sans-serif;width:48mm;margin:0 auto;padding:2mm 0;color:#111;background:#fff;font-size:11px}
            .inv{width:100%}
            .header{text-align:center;border-bottom:1.5px solid #000;padding-bottom:2mm;margin-bottom:2mm}
            .shop{font-size:13px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
            .sub{font-size:9px;color:#333}
            .meta{display:flex;justify-content:space-between;font-size:10px;padding:1.5mm 0;border-bottom:1px dashed #999}
            table{width:100%;margin-top:2mm;font-size:10px;border-collapse:collapse;border-top:1px solid #333}
            thead th{background:#eee;font-size:9px;padding:1mm 0}
            tbody td{border-bottom:1px solid #ddd;padding:1mm 0}
            .totals{margin-top:2mm;text-align:right;font-size:11px}
            .totals > div{margin-bottom:1mm}
            .tax{font-size:10px;color:#475569}
            .grand{background:#000;color:#fff;font-weight:800;padding:1.5mm;font-size:12px}
            .footer{text-align:center;margin-top:3mm;font-size:10px;color:#555}
            @media print{body{width:48mm;padding:0}}`;
    }
    ...
    Object.assign(App, {
        ...
        exportInvoicesCSV, importInvoicesCSV, sheetSync, fullSync,
