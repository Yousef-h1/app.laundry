...
(function (window) {
    ...
    async function sendMedia(cfg, number, mediaDataUrl) {
        const c = resolveCfg(cfg);
        const clean = cleanNumber(number, c.countryCode);
        if (!clean) throw new Error('Invalid number');
        ...
        const res = await fetch(url, {
            method: 'POST',
            headers: headers(cfg),
            body: JSON.stringify({ number: clean, mediatype: 'image', media: mediaDataUrl, caption: '', delay: 1 }),
    ...
    async function sendInvoiceMessage(cfg, inv, shopName, itemsText) {
        ...
        let mediaOk = true, mediaErr = '';
        try {
            const png = await buildInvoiceImage(inv, cfg);
            await sendMedia(cfg, inv.phone, png);
        } catch (e) {
            mediaOk = false;
            mediaErr = (e && e.message) ? e.message : String(e);
        }
        return { textOk: true, mediaOk, mediaErr };
    }
    ...
    const WA = {
        cleanNumber, testConnection, sendText, sendMedia,
        sendReadyMessage, sendInvoiceMessage,
        sendReminderMessage, sendSubReminderMessage,
        itemsText, drawInvoicePng, fillTemplate, normalizeServerUrl
    };
