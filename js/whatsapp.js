...
(function (window) {
    ...
    function drawInvoicePng(inv, cfg) {
        ...
        let h = 26;
        if (cfg && cfg.shopName) h += 38;
        if (cfg && cfg.shopAddr) h += 22;
        if (cfg && (cfg.crNo || cfg.shopPhone)) h += 22;
        h += 22;
        h += 24;
        if (inv.custName || inv.phone) h += 24;
        h += 12;
        h += 24;
        h += items.length * 28;
        h += 14;
        ...
        h += 28;
        if (inv.status === 'paid') h += 44;
        if (cfg && cfg.evoPayNote) h += 22;
        h += 30;
        ...
        ctx.font = '13px ' + F;
        ctx.textAlign = 'center';
        if (cfg && cfg.shopAddr) { ctx.fillText(String(cfg.shopAddr), C, y); y += 22; }
        ...
        const colName = 188, colUrg = 226, colQty = 282;
        ctx.fillStyle = '#e2e8f0';
        ctx.fillRect(X, y - 18, R - X, 24);
        ctx.fillStyle = '#000';
        ctx.font = '700 13px ' + F;
        ...
        items.forEach(item => {
            ...
            if (item.mode) {
                ctx.font = '10px ' + F;
                ctx.fillText(String(item.mode), colUrg, y - 5);
            }
            ctx.font = '700 10px ' + F;
            ctx.fillText(urg(item.urgent), colUrg, y + 7);
            ctx.font = '12px ' + F;
        ...
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        if (ctx.roundRect) ctx.roundRect(X, y - 30, R - X, 28, 3); else ctx.rect(X, y - 30, R - X, 28);
        ctx.fill();
        ctx.strokeStyle = '#111';
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ...
        ctx.font = '800 16px ' + F;
        ctx.textAlign = 'right';
        ctx.fillText((ar ? 'الإجمالي النهائي: ' : 'Total: ') + fmt(inv.netTotal) + ' ' + cur, R - 6, y - 9);
        y += 20;
        if (inv.discount > 0) {
            ctx.fillStyle = '#000';
            ctx.font = '12px ' + F;
