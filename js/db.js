...
const DB_NAME = 'laundry-pro-db';
    ...
    const DEFAULT_SETTINGS = {
        ...
        evoServerUrl:   { value: 'https://evo.awj-bh.cloud' },
        evoInstance:    { value: '35310880' },
        evoApiKey:      { value: '9B9B0781DD10-4E76-BB20-6CCB89E5C90F' },
        ...
        msgInvoice:     { value: '*{shop}* 🧺\nعزيزي العميل: *{name}*\nتم استلام طلبك بنجاح ✅\n\n*تفاصيل الخدمات (جدول):*\n──────────────────\n{items}\n──────────────────\n\n*بيانات الفاتورة:*\n🔢 رقم الفاتورة: #{inv}\n📅 التاريخ: {date}\n💰 *الإجمالي النهائي: {total} {currency}*\n\n💡 {paynote}\n\n🔗 يمكنك عرض فاتورتك الرقمية من هنا:\n{paylink}\n\nشكراً لثقتكم بنا! {shop} ✨' },
        msgReminder:    { value: '' },
        msgSubReminder: { value: '' },
        evoPayNote:     { value: 'يمكن الدفع وقت الاستلام عبر تطبيق بنفت (BenefitPay) : 35310880' },
        evoInvLink:     { value: 'https://script.google.com/macros/s/AKfycbxfJ_kVM7L6nsesW6EJhsnMzkEB3Z3bTPdatH0N5JUREGr1YUdcHQRonADJe8pINF1J/exec?id={inv}' },
        evoPushUrl:     { value: 'https://script.google.com/macros/s/AKfycbxfJ_kVM7L6nsesW6EJhsnMzkEB3Z3bTPdatH0N5JUREGr1YUdcHQRonADJe8pINF1J/exec' },
        gsheetUrl:      { value: '' }
    };
    ...
    function _seedFallback() {
        try {
            if (localStorage.getItem('laundry_pro2_settings') === null) {
                ...
                const arr = _lsList('settings');
                const map = {};
                arr.forEach(r => { map[r.key] = r.value; });
                let touched = false;
                if (map.evoServerUrl === 'http://localhost:8080') { map.evoServerUrl = DEFAULT_SETTINGS.evoServerUrl.value; touched = true; }
                if (map.evoInstance === 'default') { map.evoInstance = DEFAULT_SETTINGS.evoInstance.value; touched = true; }
                if (!map.evoApiKey) { map.evoApiKey = DEFAULT_SETTINGS.evoApiKey.value; touched = true; }
                if (touched) {
                    const out = Object.keys(map).map(k => ({ key: k, value: map[k] }));
                    _lsSave('settings', out);
                }
            }
    ...
    async function seedIfEmpty(db) {
        ...
        if (settingCount === 0) {
            ...
            // migrate known-old Evolution API defaults so an existing install links straight away
            if (have.evoServerUrl === 'http://localhost:8080') store.put({ key: 'evoServerUrl', value: DEFAULT_SETTINGS.evoServerUrl.value });
            if (have.evoInstance === 'default') store.put({ key: 'evoInstance', value: DEFAULT_SETTINGS.evoInstance.value });
            if (!have.evoApiKey) store.put({ key: 'evoApiKey', value: DEFAULT_SETTINGS.evoApiKey.value });
            if (have.evoInvLink === 'https://script.google.com/macros/s/AKfycbxDyzuRk-V-I3xxgAeH8-uZx3n4i8XUC18JwOhBn-pLVoDtftsWiNYqHW4VxNBiiflo/exec?id={inv}') store.put({ key: 'evoInvLink', value: DEFAULT_SETTINGS.evoInvLink.value });
            if (have.evoPushUrl === '') store.put({ key: 'evoPushUrl', value: DEFAULT_SETTINGS.evoPushUrl.value });
            await txDone(tx);
        }
    ...
    async function upsertMany(storeName, rows) {
        if (!Array.isArray(rows) || !rows.length) return 0;
        const db = await openDB();
        if (_useFallback) {
            rows.forEach(r => _fbPut(storeName, r));
            return rows.length;
        }
        const tx = db.transaction(storeName, 'readwrite');
        const store = tx.objectStore(storeName);
        rows.forEach(r => { if (r && r.id !== undefined) store.put(r); });
        await txDone(tx);
        return rows.length;
    }
